...
max_weeks <- max(sx_schedule$week)
...
server <- function(input, output, session) {
  ...
  output$weekly_chart <- renderPlotly({
    ...
    if (input$weekly_chart_selection == "Scores") {
      ...
      if (is.null(input$team_weekly)) {
        ...
        tm <- filter(sx_scores, team %in% input$team_weekly)
      ...
      if (is.null(input$team_weekly)) {
        ...
        tm <- filter(sx_fvoa_season, team %in% input$team_weekly)
      ...
      if (is.null(input$team_weekly)) {
        ...
        tm <- filter(sx_simulated_records, team %in% input$team_sims)
  ...
  output$playoff_leverage <- renderPlot({
    plot_playoff_leverage(sx_simulated_standings) + 
  ...
  output$points_luck <- renderPlot(plot_points_luck(sx_schedule, sx_scores, x = "pf"),
                                   res = 96)
  ...
  output$manager <- renderPlot(sx_lineup_eval, res = 96)
  ...
  output$eval_fvoa_team = renderPlot({
    plot_model_eval_team(sx_model_eval) + 
      scale_fill_manual(values = fvoa_colors)
  }, res = 96)
