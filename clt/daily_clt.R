...
clt_team_tmp <- get_team(weeks_played, 'yahoo', leagueID = 175582, season = current_season) %>% 
...
clt_wp_tmp <- get_win_prob(current_week, leagueID = 175582) %>% 
...
clt_current_matchups <- compare_current_matchups(clt_schedule, clt_fit, clt_wp, quality = T, clt_simulated_standings) %>%
  select(-Yahoo)
clt_playoffs <- compare_playoff_teams(clt_fit, "David", "Diaz", "PFinn", "Eric")
